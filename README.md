# Whisper×ELYZA×BudouXで高精度SRT字幕生成スクリプト
...
- **[新発想] タイムスタンプ維持型フィラー除去**: LLMに渡す前の段階で、「えっと」「あのー」といった言葉のヒゲ（フィラー）の時間データ（箱）は残したまま「空文字」に置換。音ズレを100%発生させずにノイズをカットします。
- **[高度なAI校正] 動画の全体像＋辞書の完全同期**: 校正をかけるローカルLLM（ELYZA）に対し、事前に「動画全体のあらすじ（要約）」と「専門用語辞書（正解リスト）」をインプット。文脈を完璧に理解した上で超高精度な表記補正を行います。
...
├── src/
    ├── __init__.py
│   ├── transcriber.py # 【第1工程：耳＋下準備】音声認識、辞書読み込み、フィラー空文字化
│   ├── llm_checker.py # 【第2工程：頭脳】全体像把握、辞書を含めた文脈バッチ校正（ELYZA）
...
├── data/              # 各種データ（※.gitignoreにより、フォルダ内の大容量メディアはGit管理外）
...
```powershell
# 特定の音声ファイルを指定
uv run whisper-llm-srt ./data/audio.mp3

# 動画ファイルを指定して直接実行
uv run whisper-llm-srt ./data/input_movie.mp4

# 超高精度モデル（large）を指定
uv run whisper-llm-srt ./data/test.m4a -m large

# 別の単語辞書ファイルを指定
uv run whisper-llm-srt ./data/test.m4a -d ./data/my_dict.txt
```
...
認識率を上げたい固有の単語やYouTubeのチャンネル名などがある場合は、data フォルダ内に dictionary.txt を作成し、1行に1単語ずつ記述してください。
登録された単語は、Whisperのプロンプトだけでなく、llm_checker（ELYZA）への「正解リスト」としても完全同期して共有されるため、強力な超能力補正が働きます。
...
1行の文字数上限や、既定のファイル名を変えたい場合は、スクリプト（main.py）上部の【初期設定エリア】を直接書き換えてください。
...
DEFAULT_DICT_FILE = "./data/dictionary.txt"  # 優先的に認識させたい固有名詞・専門用語を並べたテキストファイル
...
各モジュール（src/formatter.py）側で、字幕レイアウトの防衛ラインを調整することも可能です。
...
本プロジェクトは **MIT ライセンス** の下で公開されています。商用利用、修正、配布、プライベート利用を含め、誰でも自由に無償で利用することができます。詳細は同梱 of `LICENSE` ファイルをご確認ください。