...
認識率を上げたい固有の単語やYouTubeのチャンネル名などがある場合は、`data` フォルダ内に `dictionary.txt` を作成し、1行に1単語ずつ記述してください。登録された単語は、Whisperの初期プロンプトとして渡されます。
...
除去したいフィラー語がある場合は、`data` フォルダ内に `filler.txt` を作成し、1行に1語ずつ記述してください。ファイルが存在しない場合は、デフォルトのフィラーリスト（えっと・あの・あのー・えー等）が自動で使われます。
