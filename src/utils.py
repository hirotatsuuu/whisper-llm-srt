...
def build_output_paths(output_srt_path: str) -> dict:
    ...
    命名規則:
        - Whisper SRT     : output/whisper/<stem>_whisper.srt
        - Whisper JSON    : output/whisper/<stem>_whisper.json
        - Whisper テキスト: output/whisper/<stem>_whisper.txt
        - Elyza SRT       : <元のパスと同じ場所>（output_srt_path そのまま）
        - Elyza テキスト  : output/elyza/<stem>_elyza.txt
    ...
    elyza_dir   = os.path.join("output", "elyza")

    return {
        "elyza_srt"   : output_srt_path,
        "elyza_txt"   : os.path.join(elyza_dir,   f"{stem}_elyza.txt"),
        "whisper_srt" : os.path.join(whisper_dir,  f"{stem}_whisper.srt"),
        "whisper_json": os.path.join(whisper_dir,  f"{stem}_whisper.json"),
        "whisper_txt" : os.path.join(whisper_dir,  f"{stem}_whisper.txt"),
    }