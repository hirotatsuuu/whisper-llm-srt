...
from src.config import MAX_CHAR_LEN, MIN_CHAR_LEN
...
def _split_segment_to_lines(
    segment: dict,
    min_len: int = MIN_CHAR_LEN,
    max_len: int = MAX_CHAR_LEN,
) -> list[dict]:
...
def write_srt_file(segments: list, output_srt_path: str) -> None:
    """10〜20 文字に切り分けられた全セグメントを SRT 規約に沿ったファイルとして書き出す関数。
    ...
    try:
        ...
        with open(output_srt_path, "w", encoding="utf-8") as f:
            for segment in segments:
                ...
                split_lines = _split_segment_to_lines(segment)
